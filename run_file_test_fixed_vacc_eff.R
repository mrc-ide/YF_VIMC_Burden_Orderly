...
#Set up inputs
scenarios = paste0("2026_alt_vacc_eff_test_",c("01_novacc","02_default_routine","03_default_routine_campaign"))
...
country_list_test=c("ETH","NGA")
country_list_file = paste0(path,"/shared/country_list_test.csv")
write.csv(data.frame(country=country_list_test),file=country_list_file,row.names=FALSE)
input_data_ids=rep("",length(scenarios))
vaccine_efficacy_data_file_alt = paste0(path, "/shared/vacc_eff_200_values_alt.Rds")
vaccine_efficacy_alt = rep(0.975,200)
saveRDS(vaccine_efficacy_alt, file = vaccine_efficacy_data_file_alt)
for(n_scn in 1:length(scenarios)){
  input_data_ids[n_scn] = orderly2::orderly_run("01_input_data_setup", 
                                            list(scenario_name = scenarios[n_scn],
                                                 ...
                                                 vaccine_efficacy_median = 0.975, 
                                                 vaccine_efficacy_data_file = vaccine_efficacy_data_file_alt, 
