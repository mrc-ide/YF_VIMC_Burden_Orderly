...
#-------------------------------------------------------------------------------
# Image population or vaccination data for one region
image_selected_region_data <- function(matrix=matrix(),years=c(2000),ages=c(0:100),
                                       colour_scale=hcl.colors(12, "YlOrRd", rev = TRUE),title=""){
  assert_that(is.array(matrix))
  assert_that(length(dim(matrix))==2)
  assert_that(is.numeric(years))
  assert_that(is.numeric(ages))
  assert_that(dim(matrix)[1]==length(years))
  assert_that(dim(matrix)[2]==length(ages))
  
  image(x=years,y=ages,z=matrix,zlim=c(0,max(matrix)),xlab="Year",ylab="Age",col = colour_scale)
  title(main=title)
